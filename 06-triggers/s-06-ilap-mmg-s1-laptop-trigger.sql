...
--@Descripción: Definición del trigger para operaciones DML para LAPTOP en MAMGBD_S1

create or replace trigger t_dml_laptop_mmg_s1
...
begin
    ...
    when inserting then
      ...
      -- Inserción local (MAMGBD_S1)
      if (v_num_serie = '4' or v_num_serie = '5') then
    ...
    when deleting then
      ...
      -- Eliminación local (MAMGBD_S1)
      if (v_num_serie = '4' or v_num_serie = '5') then
        delete from LAPTOP_F4 where laptop_id = :old.laptop_id;
      -- Eliminación remota (CAHABDD_S1)
      elsif (v_num_serie = '0' or v_num_serie = '1') then
